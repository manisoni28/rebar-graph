...
public class AsgScanner extends AwsEntityScanner<AutoScalingGroup, AmazonEC2Client> {
	...
	public static class AsgRelationshipGraphOperation implements GraphOperation {
		...
		public Stream<JsonNode> exec(Scanner ctx, JsonNode n, GraphDriver neo4j) {
			...
			neo4j.cypher(cypher).param("arn", arn).param("region", region).param("account", account)
					.param("instances", instances).exec();
			...
			neo4j.cypher(cypher).param("arn", arn).param("region", region).param("account", account)
					.param("names", loadBalancerNames).exec();
	...
	protected void project(AutoScalingGroup asg) {
		...
		n.set("vpcZoneIdentifier", Json.objectMapper().valueToTree(
				Splitter.on(",").omitEmptyStrings().trimResults().splitToList(asg.getVPCZoneIdentifier())));
