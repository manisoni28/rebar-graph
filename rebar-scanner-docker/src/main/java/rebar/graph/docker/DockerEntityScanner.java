...
public abstract class DockerEntityScanner<OBJECTTYPE> extends EntityScanner<DockerScanner, DockerEntityType, OBJECTTYPE, DockerClient>{
	...
	protected void gc(DockerEntityType type, long ts) {
		getScanner().getGraphDB().getNeo4jDriver().cypher("match (a:"+type.name()+" {hostId:{hostId}}) where a.graphUpdateTs<{ts} return a").param("ts", ts).param("hostId", getHostId()).forEach(it->{
			scan(it);
		});
	}
}
