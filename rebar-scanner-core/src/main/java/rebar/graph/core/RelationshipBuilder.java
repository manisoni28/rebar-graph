...
public class RelationshipBuilder {
	...
	private void mergeRelationships() {
		...
		String cypher = String.format("match (a:%s %s), (b:%s %s)", aLabel,
				toMatchPattern(sourceIdAttributes,"a_"), bLabel,
		...
		cypher = cypher + String.format(" %s merge (a)-[r:%s]->(b) return count(r) as count",whereClause,relationshipName);
	...
	private void deleteStaleRelationships() {
		...
		cypher = cypher + String.format(" where NOT (b.%s in a.%s) delete r return count(r) as count",bJoinAttributeName,aJoinAttribute);
